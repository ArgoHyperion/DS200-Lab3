...
users = sc.textFile("users.txt")     .map(lambda x: x.split(","))     .map(lambda x: (x[0], age_group(x[2])))

movies = sc.textFile("movies.txt")     .map(lambda x: x.split(","))     .map(lambda x: (x[0], x[1]))   # movie_id -> title
